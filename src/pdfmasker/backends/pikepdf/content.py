...
def iter_segments(instructions: Sequence, fonts: Mapping[str, Font]) -> Iterator[list[_Chunk]]:
    """Yield each flushed segment: a run of show-text ops under one font.

    A segment is bounded by font changes and text-block markers, so masking and read-only extraction can share this
    single walk instead of duplicating it. Each yield is a fresh list the caller may consume freely — nothing is
    retained here.
    """
    raw_font = Font.raw()
    ...
    for index, instruction in enumerate(instructions):
        ...
        if operator in ("Tf", "BT", "ET"):  # font change or text-block marker ends a segment
            if segment:
                yield segment
                segment = []
            if operator == "Tf" and len(operands) >= _TF_MIN_OPERANDS:
...
def process(
    ...
    for segment in iter_segments(instructions, fonts):
        _apply_segment(segment, replacements, counts, changes)
    return _rebuild(instructions, changes), counts, bool(changes)


def extract_segments(container: pikepdf.Object | pikepdf.Page, fonts: Mapping[str, Font]) -> Iterator[str]:
    """Yield the reconstructed text of each non-empty segment, without modifying anything.

    This is the read-only half of `process`: the same walk and reconstruction a detector reads, with no matching or
    write-back.
    """
    instructions = list(parse_content_stream(container))
    for segment in iter_segments(instructions, fonts):
        full, _ = _reconstruct(segment)
        if full:
            yield full
